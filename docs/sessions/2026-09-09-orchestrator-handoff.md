...
| `/Users/ahmed/Dev/fo-wt-072` | `task/TASK-072-seed-schemas-dataset-skeleton` · no PR yet | Spec 006's seed schemas + generated dataset skeleton (ADR-0017 projections of spec 005's dataset). | **DONE — PR #38 ready, CI spine green** (head rebased before ready): 6 schema files, `pnpm seed:project --check` byte-determinism proof, 84/23/32/6 rows + 126 `occasion_country` rows, 62 new unit tests, 2 002 suite. Next: light orchestrator review (internal data/schemas) → merge → mark TASK-072 `done` → dispatch TASK-073 and TASK-077 (both depend only on 072); TASK-074 also needs PR #37. Two items for the orchestrator to rule on in the PR body: projected files carry authored camelCase records (not `to*Row()` output) and taxonomy/categories/occasions are generated too; a `plan/03` §9 gap — Romanian Orthodox Easter and Polish name days need a seventh occasion rule type (spec 009 amendment). |
