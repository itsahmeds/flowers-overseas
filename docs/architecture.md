...
| **Derived image bytes are committed to the repository** — `public/media/` holds 118 AVIF/WebP files, 3 012 746 B (47.9 % of the 6 MB cap spec 006 §13 Q4 set), derived from 31 originals that are **not** committed. A build artefact in version control is the thing this repository otherwise refuses; it is accepted here because there is no object store yet, the demo needs pictures on 16 Oct, and a capped, checksummed, gate-enforced 3 MB is cheaper than provisioning R2 to show a florist a homepage. The cap is enforced before the bytes are served (`pnpm seed:check` rule family 9), the manifest is the only source of widths, bytes and checksums (`pnpm media:variants --check`), and the Phase-0 ladder is narrowed per slot (`PHASE0_SLOT_WIDTHS`) rather than shipping the full §13 Q5 ladder. Accepted 2026-09-18 for four weeks (TASK-080). | spec 006 §2.6 / TASK-083 | The R2 flip: `r2VariantLoader` replaces `staticVariantLoader` inside `src/modules/ui/media/`, the `media.derive_variants` job produces the full ladder once, and **`public/media/` and `staticVariantLoader` are deleted in the same PR** so there is never more than one image origin (spec 006 AC-27). |
