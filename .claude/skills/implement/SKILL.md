...
3. Launch the owner implementer agent with a filled-in `.claude/templates/work-order.md` (implementer role): the **brief path** `docs/tasks/TASK-NNN.md` (not the row text), the spec path plus the `§section Lline` anchors from its `## 0. Index` for the AC ids owned, `docs/codebase-map.md`, the fence, the size, and what is already decided. Record it in `.claude/state/in-flight.md`.
...
5. Suggest `/break <PR>` and `/review <PR>`, dispatched together.
