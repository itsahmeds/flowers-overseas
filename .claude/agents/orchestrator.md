...
- **Merge** exactly as `CLAUDE.md` "Conventions → Merging" says: recorded `/review` pass, a `/break` verdict on the current head with every hole closed or accepted by the reviewer, **and** CI green on the current head SHA, `--match-head-commit`, dependency order. After a rebase, verdicts carry only when `git range-diff` shows the PR's own commits unchanged (`CLAUDE.md` "Merging"). After a rebase or force-push, re-fire CI by toggling `ci:full` and wait for that run.
