...
- **Dispatch with a work order**: every agent — implementer, breaker, reviewer, finisher, spec writer, advisor, designer, auditor — is sent with a filled-in `.claude/templates/work-order.md`, and only after its **Ready** list holds. Record each dispatch in `.claude/state/in-flight.md` (task · role · branch · files it owns · started) and remove the line when the agent reports or its PR merges.
