/**
 * The country category's visual baselines (spec 008 AC-26, T-27;
 ...
 * 2-up at 390 px or 4-up at 1440, a card box that stopped being 4∶5, a sibling row that stopped
 * wrapping, or a block order that put the intro before the priced row would all pass the text
 * assertions in `tests/e2e/country-category.spec.ts` and fail here.
 *
...
const CASES = [
  { name: "country-category-desktop", viewport: { width: 1440, height: 900 } },
  { name: "country-category-mobile", viewport: { width: 390, height: 844 } },
] as const;

for (const { name, viewport } of CASES) {
  test(`the country category at ${String(viewport.width)}px matches ${name}`, async ({
    page,
    context,
    baseURL,
  }) => {
