...
describe("the committed TASKS.md (AC-31 / T-32)", () => {
  ...
  it("reports Phase 0 as 10 / 12 specs approved", () => {
    ...
    expect(PHASE_0_SPECS_APPROVED).toBe("10 / 12");
  });
...
const fixture = [
  ...
  "|---|---|---|---|",
  "| 0 Demo | 10 / 12 | 12 / 43 | in progress |",
  "",
  "## Tasks",
  `| ${TASK_COLUMNS.join(" | ")} |`,
...
describe("checkLedger failure paths", () => {
  ...
  it("reports a Phase 0 progress row that no longer says 10 / 12", () => {
    const broken = fixture.replace(
      "| 0 Demo | 10 / 12 |",
      "| 0 Demo | 5 / 12 |",
    );
    expect(checkLedger(broken).join("\n")).toContain(
      'Phase 0 specs approved "5 / 12"',
    );
  });
