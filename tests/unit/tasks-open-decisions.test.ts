...
describe("the committed TASKS.md (AC-31 / T-32)", () => {
  ...
  it("reports Phase 0 as 4 / 12 specs approved", () => {
    ...
    expect(PHASE_0_SPECS_APPROVED).toBe("4 / 12");
  });
...
describe("checkLedger failure paths", () => {
  ...
  it("reports a Phase 0 progress row that no longer says 4 / 12", () => {
    const broken = fixture.replace(
      "| 0 Demo | 4 / 12 |",
      "| 0 Demo | 5 / 12 |",
    );
