...
- **Signal.** §6's deliberate narrowing ("language preferences only, no IP read") is reversed as `plan/03` §2 originally allowed: the hint is the **best match of `navigator.languages`, then the edge country header** (`cf-ipcountry` behind Cloudflare, `x-vercel-ip-country` on the cold fallback), mapped country → locale by one table in `src/config/` (`DE`/`AT`/`CH-de` → `de`, `PL` → `pl`, `GB`/`IE` → `en-gb`, everything else → `en`). The country is read **only** by `hints.ts` (the one file `fo/no-geo-redirect` allows), exposed through a tiny `GET /api/geo` route (`Cache-Control: no-store`, body `{ "country": "DE" }`, no IP stored or logged) that the client island fetches once when no `fo_locale` cookie exists. Cached HTML stays byte-identical for everyone: no `Vary`, no `Set-Cookie` on a page, no bot branch (AC-7/AC-9 unchanged).
- **Shape.** A native `<dialog>` with `role="dialog"`, `aria-labelledby`, `Esc` to stay, focus moved to the primary action and returned on close; **desktop** a centred card over a dimmed page; **mobile** a compact bottom sheet covering ≤ 35 % of the viewport so the main content stays visible (Google's intrusive-interstitial rule for first visits from search). Copy in the *suggested* locale's language with the current locale's line beneath. Two actions only — *Continue in {native name}* (sets `fo_locale`, navigates by a real `<a>` to the same path in the other locale) and *Stay in {current}* (sets `fo_locale` to the current locale). Never shown again once either is chosen; hidden when the hint equals the URL locale or when no better launch locale exists. Shown **before** the consent sheet, which waits for the dialog to close; zero layout shift (overlay, reserved nothing). Lazy-imported after hydration; budget +≤ 3 KB Brotli against spec 004's script budget, measured.
- **Compliance.** Reading a country from the IP is processing of personal data: RoPA gains a row (purpose: language suggestion; basis: legitimate interest; data: IP-derived country code only, in memory, never stored; retention: none). EU 2018/302 is satisfied because the visitor chooses and nothing is blocked or redirected.
- **Owner.** TASK-119. Spec 004's `LocaleSuggestionBanner` markup is replaced, its message keys renamed; `decideSuggestion()` gains the country input with a unit table; T-09-style hash equality on `/en` with and without `Accept-Language` and with and without `cf-ipcountry` is re-asserted.
