...
**A15 — Agent orientation: the codebase map, per-task brief files and section-anchored briefs (§2 "Dev OS"; new AC-33…AC-36; TASK-086).**
Original: §2 gives every agent the same orientation path — `CLAUDE.md` → the full `TASKS.md` row → the whole spec → `docs/architecture.md` — and every task row grows by appending carry-forwards from each `/review`.
Measured (orchestrator, 2026-09-11): `TASKS.md` is 344 KB with task rows up to 12 KB; specs 004/005 are 99/78 KB; `docs/architecture.md` is 41 KB. Each subagent loads 30–60k tokens of orientation before its first useful action, and six agents exhausted the Opus session limit in one afternoon mostly re-reading the same files. Twice the ledger test went red on an unescaped `|` inside an appended carry-forward.
Corrected (founder approval 2026-09-11, orchestrator spec): the Dev OS gains three orientation artefacts, each kept current by a test.
- **AC-33 — codebase map.** `docs/codebase-map.md` is generated by `pnpm codebase:map` (`scripts/codebase-map.ts`) from `src/modules/*/index.ts` barrels, `src/config/*`, `src/app/**` route files, `scripts/*.ts` (with their `package.json` script names) and `tests/<layer>/`: one line per entry with its purpose (first doc-comment line or a `@purpose` tag; missing → `TODO purpose` and the check fails), owning spec (`@spec NNN` tag or the module boundary table in `plan/01` §5) and its test files (by `import` of the module, resolved statically). It ends with a hand-maintained "Where does X live?" table (money, honesty gates, budgets, consent, i18n formatting, media placeholders, ledger parser, design source of truth) that the generator preserves verbatim. `pnpm codebase:map --check` exits non-zero when the committed file differs from the generated one; a unit test runs the check. Target size ≤ 12 KB.
- **AC-34 — per-task brief files.** Every task row in `TASKS.md` keeps exactly nine cells; its "Blockers / notes" cell is at most 400 characters and links `docs/tasks/TASK-NNN.md` where the binding clauses, the exact files to read, the carry-forwards (one dated bullet per `/review`), and the escalation record live, under fixed headings (`## Binding`, `## Read`, `## Carry-forwards`, `## Escalations`, `## Result`). The migration moves every existing row's notes into its brief file losslessly (a test diffs the concatenated notes before and after for every task id). `scripts/tasks-open-decisions.ts` gains the 400-character check and a check that every `todo`/`in_progress`/`in_review` task has a brief file; `pnpm tasks:brief TASK-NNN` scaffolds one from the template `docs/tasks/_template.md`.
- **AC-35 — section-anchored specs.** Each spec under `specs/` gets an `## 0. Index` block after the header table listing every `AC-n` and `T-n` with its section and line-anchor (`§9 L244`), regenerated by `pnpm specs:index` and checked the same way as the map; the spec template gains the block.
