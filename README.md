...
| `pnpm check:no-db` | no file in the scanned set (`src/config/`, `src/modules/i18n/`, `src/modules/catalog/`, `seed/schema/`, `seed/project.ts`, `seed/media-variants.ts`) imports a database client, an ORM or a Postgres driver, or reads `DATABASE_URL` |
