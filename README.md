...
| `pnpm codebase:map` | regenerates `docs/codebase-map.md` — one line per module barrel, `src/config/` module, `src/app/**` route file and `scripts/*.ts` with its purpose (first doc-comment line or `@purpose` tag), owning spec and the test files that import it, plus the test-layer counts. The hand-maintained "Where does X live?" table at the end is copied through verbatim. `--check` fails when the committed file is stale or a file has no purpose (spec 001 §14 A15, AC-33) |
| `pnpm specs:index` | inserts or refreshes the `## 0. Index` block under each spec's header table: every `AC-n` and `T-n` with the section and line it is defined at (`§9 L244`). `--check` fails on a stale or missing block (AC-35) |
| `pnpm tasks:check` | the `TASKS.md` ledger gate: the `plan/13` §A open decisions, the Phase 0 progress row, nine cells per task row with escaped pipes, known statuses, the 400-character cap on the "Blockers / notes" cell and a brief file for every `todo`/`in_progress`/`in_review` row (AC-31, AC-34) |
| `pnpm visual:baselines` | the visual-baseline ledger: `--check` fails when a screenshot baseline exists for `darwin` but not for `linux`, which is the only platform the `visual` job compares against (it runs in the `lint` job); `--manifest` writes the sha256 of every `linux` baseline, and `--verify <manifest>` proves the committed PNGs are byte-for-byte what a `visual-baselines` run produced (spec 001 §2, AC-17; `docs/runbooks/visual-baselines.md`) |
| `pnpm tasks:brief` | invoked as `pnpm tasks:brief TASK-NNN`: scaffolds `docs/tasks/TASK-NNN.md` from `docs/tasks/_template.md` with the five fixed headings (`Binding`, `Read`, `Carry-forwards`, `Escalations`, `Result`) |
| `pnpm tasks:migrate` | idempotent: moves any row whose notes cell is still long-form prose into its brief and leaves the cell as `Brief: \`docs/tasks/TASK-NNN.md\`. <first sentence>`. Lossless — a row whose brief does not reproduce its notes is reported and left alone. Re-run it after a merge conflict in `TASKS.md` |
| `pnpm check:no-literal-disable` | no file under `src/` disables `fo/no-literal-strings` |
| `pnpm check:no-vercel-env` | no file under `src/`, `scripts/` or `tests/` reads `VERCEL_ENV`, `NEXT_PUBLIC_VERCEL_ENV` or `VERCEL_GIT_COMMIT_SHA` outside `src/lib/env.schema.ts` and `src/lib/sentry.ts` — the abstraction `appEnvironment()` / `hostPlatform()` / `commitSha()` is the only way the app learns where it runs (spec 040 AC-2) |
| `pnpm check:no-db` | no file in the scanned set imports a database client, an ORM or a Postgres driver, or reads `DATABASE_URL`. The set is `SCANNED_PATHS` in the script and is explicit on purpose: `src/config/`, `src/modules/i18n/`, `src/modules/catalog/`, `src/modules/ui/`, `src/modules/analytics/`, the spec-004 route tree (`src/app/(chooser)/`, `src/app/(dev)/`, `src/app/[locale]/`, the three document files, `robots.ts`, `api/consent`, `api/csp-report`), the §5.2 server seams (`src/lib/csp.ts`, `consent.ts`, `consent-bootstrap.ts`, `robots-headers.ts`, `media-headers.ts`) and the offline half of `seed/`. `src/app/api/` is **not** listed as a whole: spec 013's checkout and spec 012's admin will import the client there on purpose (spec 004 AC-2) |
...
| `pnpm seed:check` | the seed-dataset gate (spec 006 AC-10, AC-30): the nine rule families of spec 006 §2.3 over `seed/data/**` — every file parsing under its own schema with its `version`/`source`/`origin` header and no projected file hand-edited; the counts and the 40/14/8/10/12 product split; referential integrity (facet values, category and occasion edges, a price row per seeded destination, a media asset's product and its prompt hash); slugs (ASCII, lowercase, hyphenated, no trailing slash, the ASCII fold of the name, unique per locale **across** products, categories and occasions); prices, through the same band, ending, tier-step and single-open-ended-row checks the catalogue gate above uses, run over the rows in the files; copy (60–90 words, the closing local-florist sentence, banned superlatives, no duplicate description, no delivery-timing claim); media (one primary per product, no `delivery` asset, and — once a variants manifest or `alt/` exists — variants present with matching bytes and alt text per launch locale); **no PII and no third-party mark** anywhere, reported with the file and the JSON path; and the per-slot imagery byte budgets, read from `seed/data/media-variants.json` (TASK-138: the derived bytes live in the media bucket, so the gate reads the committed manifest rather than a committed tree). One line per problem naming the file, the entity key and the rule. `--report` prints the standing catalogue-health report of spec 006 §11 (product counts, the per-(country, category) and per-(country, occasion) six-product coverage table, price extremes, the word-count distribution, copy review shares per locale, the manifest's image-byte total, per-slot maxima, products still on a placeholder, `seoTitle` near-duplicate pairs) and appends it to the GitHub step summary |
| `pnpm seed:diff` | the per-table diff between `seed/data/**` and a `SeedTarget` (spec 006 AC-11): inserts / updates / unchanged / skipped-real / orphans / conflicts per spec 002 §5.1 table, computed over **projected** rows through the same `to*Row()` functions the importer uses (ADR-0017), keyed on the natural key the importer upserts on. The Phase-0 target is the committed snapshot under `seed/snapshot/` (one file per table); TASK-083 adds `dbTarget` and `pnpm db:seed` prints the same report against the database. An update names the row and every changed column as `column: old -> new`, with a minor amount always beside its currency; a target row whose `source` is `real` is a **conflict and never an update** (`plan/10` §4); the report carries §11's `actor = system:seed` and `request_id` header, `n/a` until there is a request. Offline, no clock, byte-stable across runs. `--write` regenerates the snapshot after a dataset edit (and `tests/unit/seed-diff.test.ts` fails a forgotten one with the table named), `--dry-run` writes nothing, `--all` lists every changed row, `--report` appends to the GitHub step summary |
| `pnpm media:variants` | the deterministic imagery ladder (spec 006 §2.4, AC-12/13/14): for every asset in `seed/data/media.json` whose original is present under the git-ignored `.local/imagery/originals/`, it strips EXIF and GPS (keeping the ICC profile, and XMP when it carries a C2PA claim), crops to the slot's declared aspect ratio, encodes AVIF (q50/effort 4) + WebP (q72) at 384/640/828/1080/1200/1600/1920 plus one 1200 px JPEG for OG and email, writes `.local/media/{assetId}/{width}.{fmt}` and rewrites `seed/data/media-variants.json`. The pinned `sharp`/libvips versions and every encoder option are recorded in that file's header, and the encoder is held to one thread because libaom's AVIF output depends on its thread count — so two runs are byte-identical and changing one option is a whole-manifest re-derivation. A missing original is reported, never fatal. `--only <assetId>` derives one asset; **`--check` verifies the manifest always and the files where they exist** — since TASK-138 the derived bytes are git-ignored under `.local/media/` and live in the media bucket, so on a clean clone and in CI it checks the pinned pipeline, the canonical object keys and the declared boxes, and says that the checksum half did not run. Generation never runs in CI |
| `pnpm media:upload` | puts the derived variants into the `flowersoverseas-media` bucket (spec 006 §2.6, AC-27; TASK-138), **idempotently by checksum**: every object carries its SHA-256 as metadata, so a run `HEAD`s each key and uploads only what differs — a re-run after one new photograph transfers one photograph. No database and no queue: the manifest `seed/data/media-variants.json` is the source of truth in Phase 0. It refuses to upload unless `media:variants --check` is clean, every file is inside its slot's byte cap and no file carries the demo watermark, and unless `R2_PUBLIC_BASE_URL` equals `MEDIA_ORIGIN` (`src/lib/media-origin.ts`), the constant the site builds image URLs and its CSP `img-src` from. Each object is stored with its content type and `Cache-Control: public, max-age=31536000, immutable`. `--dry-run` plans without writing, `--only <assetId>` scopes it, `--force` re-uploads regardless, and `--verify` audits the other direction — a public `HEAD` per manifest row, comparing the published `content-length` and `content-type` to the row, with **no credential and no derived tree** (it is the only check that reads the bucket; see `docs/runbooks/imagery.md` §6). Credentials come from `.env.local` through `@next/env`; no value is ever printed |
...
| `pnpm railway:check` | the hosting drift gate, with `--env <name>` for the variable half (spec 040 AC-9 / AC-11): the `web` service's live region, replica count, healthcheck path and restart policy against `config/railway.json`, and — with `--env` — the environment's variable **key set** against the 28-key contract. Key names only, never a value. Needs `RAILWAY_API_TOKEN`, `RAILWAY_PROJECT_ID` and `RAILWAY_ENVIRONMENT_ID`, or `--fixture-environment <path>` for a recorded response (`docs/runbooks/railway-cloudflare-setup.md`) |
| `pnpm db:check` | the offline migration gate (spec 002 §2): every `db/migrations/NNNN_name.sql` has its hand-written `NNNN_name.down.sql`, versions are unique and contiguous from `0001`, every migration except `0001` opens with `SET LOCAL ROLE app_owner;` (so `app_owner` owns every object), and the tables in `db/schema/` and the tables any migration creates are the same set. One line per problem, naming the file. The connected half — column drift, RLS coverage, the `*_minor`/currency pair, `bytea`, the recipient-email ban — is AC-26 on TASK-027 |
| `pnpm db:migrate` | apply every pending migration over `DATABASE_URL_UNPOOLED` (the direct URL — spec 002 §13 Q6), each in one transaction, recorded in `public.schema_migrations`. `--dry-run` prints the plan and touches nothing |
| `pnpm db:rollback` | `--to NNNN`: roll back, newest first, down to but not including version `NNNN`, running each migration's `.down.sql` in one transaction. `--to 0000` empties the database (spec 002 AC-4) |
| `pnpm db:generate` | Drizzle Kit writes forward SQL into `db/migrations/` from `db/schema/`. The rollback file is written by hand, in the same commit |
| `pnpm db:studio` | Drizzle Studio against the direct URL |
...
| `pnpm catalogue:check` | the catalogue and price gate (spec 005 AC-5): every product has a tier and exactly one default tier, every tier of every product has exactly one active `country_price` row in every `live`/`demo` destination, no second open-ended row for one (product, country, tier, surcharge) and no overlapping closed windows, **every** tier inside its `plan/10` §2.3 band with the tiers increasing (spec 005 §14 A1: the bands hold exactly, the "~+30% / +60%" steps are approximate), each destination's authored bands equal to a transcription of `plan/10` §2.3's own table (so a band widened to make a ladder fit is caught, not accommodated), each destination's Sunday and peak-day surcharge at `plan/10` §2.3's +€4 / +€6 equivalent and taxed at the same VAT rate as the retail rows it is added to, every amount an integer minor unit on the currency's psychological ending, every add-on priced per destination and carrying the destination's *standard* VAT rate rather than its flower rate, every facet value in the `plan/10` §1.1 taxonomy, every `labelKey` present and spellable in `messages/en.json`, the euro-base FX snapshot dated and integer, and every projection's key set equal to spec 002 §5.1's columns. `--summary` appends the per-destination coverage table to the GitHub step summary |
| `pnpm corridor:check` | the corridor content gate (spec 007 AC-2): eighteen rules over `content/corridors/{locale}/{iso2}-{state}.md` — does the file parse under `CountryLocaleContentSchema`, is it `source: human` (corridor copy is never machine-drafted, `plan/02` §12), is the guide body ≥600 words and the intro 120–200, is it at least 0.80 5-gram shingle-distinct from every other corridor file in its locale (`1 − |S∩S'| / |S∪S'|` over the sets of contiguous five-token shingles — spec 007 §14 A4 re-cut AC-2's "70% token-distinct" as this, because a token multiset measures shared English vocabulary rather than shared writing), are there 8–12 FAQ items with at least half of the questions naming the country, one of its cities, its language, its currency or one of its occasions, is `seoTitle` ≤60 and `seoDescription` ≤155, does the country's own URL slug or another destination's name, adjective or slug appear in the body, does any of the nine banned words of `docs/design/README.md` §Voice appear, does a `guide`-state file claim a delivery time, a cutoff, a florist count, a rating, a review or a delivery photo, is there a digit-bearing claim of scale or a price literal, does a `live` file exist for a country with no complete `operations` block, does an `en-gb` file that `extends: en` override title, description and ≥2 FAQ answers, and are the two-or-three `relatedIso2` targets reciprocal wherever the target still has a free slot (spec 007 §14 A3: seven pages naming three others each is an odd number of edge-ends, so one edge per locale cannot be reciprocated). One line per problem naming the file, the field and the rule. `--summary` appends the published-set summary of spec 007 §11 (files, reviewed files and countries per locale) to the GitHub step summary |
| `pnpm corridor:index` | regenerate `src/modules/geo/content/corpus.generated.ts` — the typed index of `content/corridors/**` the application reads, so a corridor page pulls no `node:fs` into its bundle (spec 007 §2: "the files are imported through a generated typed index"). `--check` exits 1 when the committed index is stale; `tests/unit/corridor-corpus-index.test.ts` runs that check, so a guide edited without regenerating fails the unit suite rather than serving the previous text |
...
| `pnpm test:a11y` | Playwright `a11y` with `@axe-core/playwright`: zero serious/critical violations, **no exception list**, on the eight surfaces of spec 004 AC-26 — `/`, the four locale homes, `/dev/components`, a 404, `/ar-XB` and the 500 boundary (both documents, through `/dev/boom` and `/{locale}/boom`). The target needs `ENABLE_DEV_UI=true` and `ENABLE_PSEUDO_LOCALES=true` |
...
| `pnpm lighthouse` | Lighthouse CI with the `plan/01` §7 budgets from `lighthouserc.json` over `tests/fixtures/seo/lighthouse-urls.json` (`/`, `/en`, `/en-gb`, `/de`, `/pl`). **Required in CI since TASK-056**: performance / accessibility / best-practices ≥ 0.95, LCP < 2 000 ms, CLS < 0.05, script transfer ≤ 131 072 B, image transfer ≤ 204 800 B. Point it at a Brotli origin — `LHCI_BASE_URL=http://127.0.0.1:3001` with the proxy of the row below running — or the script number is gzip and not comparable to the budget (see the row below and `lighthouserc.json`'s note) |
| `pnpm lighthouse:origin` | a Brotli-encoding reverse proxy on `127.0.0.1:3001` in front of `next start`, so Lighthouse measures the encoding the budget is written in and the bytes this repository produces. Local sequence: `pnpm build && pnpm start`, then this in a second shell, then the row above with `LHCI_BASE_URL=http://127.0.0.1:3001`. `next start` negotiates gzip (~24 KB more on a locale document) and a Vercel preview adds `vercel.live`, which is why neither is the measurement target any more |
| `pnpm budget:client-js` | reads the prerendered documents of the last production build and prints the spec 004 §11 bundle table — per URL, the Brotli (and gzipped) size of the scripts a browser actually fetches, chunk by chunk, the delta against the committed baseline, the font transfer, plus the `localeDocument` namespace subset per locale (an upper bound since TASK-085: no message payload is sent to any client), the Client Components each document mounts by name, and whether any route ships `zod`, `@sentry/` or a `home.*`/`finder.*`/`catalog.*`/`media.*` catalogue value. Budgets: **128 KB Brotli** of JS (`plan/01` §7 as restated by spec 004 §13 Q13 and corrected by §14 A1 — transfer size, which is what Vercel serves and Lighthouse measures; `lighthouserc.json` asserts the same number), 4 KB gz of messages (spec 003 §6, AC-27), ≤45 KB of fonts (AC-4), **no more than 5 KB of growth** on any route against `tests/fixtures/seo/bundle-baseline.json`, and **zero application JavaScript on `/`** — asserted from the chunk contents, because a framework floor plus a small island looks like a framework floor. **Required in CI since TASK-056.** Exits non-zero on a breach; build first, and re-run with `--update-baseline` (deliberately, naming the import in the PR) when a number legitimately moves. `<script noModule>` (Next's legacy polyfill bundle) is listed but not counted — no module-supporting browser fetches it |
...
`src/ app/ db/ emails/ seed/ tests/` unless `.claude/state/active-task` names a task
