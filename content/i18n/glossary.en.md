...
| Product and tier names  | **Product names stay English in every locale, uninflected**: `Amber Hour`, `Vistula Red`, `Baltic Dawn`, `Kraków Spring`, `White Linen`, `Peony Note`, `Field of Gold`, `Warsaw Morning`, `Farewell Wreath`, `Moth Orchid White` — the 84 authored in `src/config/catalogue/products.data.ts` (TASK-061), under `plan/10` §2.2's `{Evocative name}` convention: the name stays, the descriptor localises. Never translate one, never add a case ending, never abbreviate. `Kraków` keeps its diacritic in prose and loses it in the slug (`krakow-spring`). **Tier names do not exist**: a tier is labelled by a message key — `catalog.tier.stems` (an ICU plural: "12 stems"), `catalog.tier.size.{s,m,l}`, `catalog.tier.single` — never by a marketing adjective (spec 005 §13 Q4), so there is nothing here for a reviewer to translate but the plural forms. |
